...
## 🧱 MVP 범위(축소 정의)
- 데이터: 일봉 OHLCV(1~5년), 관심 종목 최대 50개, 데이터 소스는 Yahoo 우선
- 지표/신호: RSI/EMA/MACD/BBANDS + 간단 규칙 결합(히스테리시스 적용)
- 모델: 베이스라인(RandomForest/LogReg)부터 시작, LSTM/GRU는 후순위
- 거래: 가상 거래만(롱), Market on Close 체결, 고정 수수료/슬리피지, 포지션 사이징 고정비율
- 백테스트: 바-클로즈, 비용/슬리피지 반영, 성과지표(Sharpe/Sortino/MDD/PF/승률)
- 프론트: 대시보드, 주식 상세, 가상 거래 3페이지 중심(MVP)

## 📦 유니버스/데이터 소스 정책
- 유니버스: 사용자 지정 심볼 리스트, 최대 50개(예: 대형주 위주). `UNIVERSE` 환경변수로 관리
- 1차 소스: Yahoo Finance(조정주가 기준, 조정배당/스플릿 반영)
- 백업 소스: Alpha Vantage(무료 티어) 또는 로컬 CSV 인제스트(스키마 고정)
- 데이터 동기화: 누락 일자 로그/재시도, 실패 시 백업 소스로 보강
- 벤치마크: 기본 `SPY`(변경 가능 `BENCHMARK_SYMBOL`)
...
Note:
- MVP 초기에는 Redis/Celery/WebSocket 미도입. 일일 배치와 동기 API 위주로 시작하고, 추후 단계에서 실시간화.
...
```
APP_ENV=local
DATABASE_URL=postgresql://user:pass@localhost:5432/stockvision
DATA_SOURCE=yahoo
ALPHA_VANTAGE_API_KEY=
UNIVERSE=AAPL,MSFT,NVDA,...
FEE_RATE=0.0005              # 0.05%
FEE_MIN=0                    # 고정 최소 수수료(선택)
SLIPPAGE_K=1.0               # 스프레드배의 배수
POSITION_RISK_PCT=0.02       # 포지션 당 리스크 비율(2%)
RISK_DAILY_LIMIT=-0.03       # 일 손실 한도(-3%)
MAX_CONSECUTIVE_LOSSES=3
MAX_SECTOR_EXPOSURE=0.3      # 섹터 편중 한도(30%)
...
가상 운용 KPI(운영 중 모니터링):
- Sharpe, Sortino, MDD, 승률, 평균손익비, Profit Factor, Turnover
- 리스크 가드레일 위반 횟수(일손실 한도, 연속 손실, 편중 한도)

## 🧪 평가 프로토콜
- 홀드아웃: 최근 6개월 고정 홀드아웃(백테스트/튜닝에 미사용)
- 교차검증: TimeSeriesSplit k=5(중첩 금지)
- 리밸런싱/체결 규칙: 일봉 MOC만, 장중 체결 없음
- 비용/슬리피지: 운영 파라미터와 동일 값 사용(현실성 일관)
- 보고서: 일일 KPI(S/MDD/승률/PF) + 에쿼티커브 + Top Trades
- 벤치마크 대비 알파/베타 산출(선택)

## 🧼 데이터 품질 규칙
- 결측: 가격 필드 결측 시 바 무시(전진 보간 금지), 지표는 해당 지점 NA 유지
- 갭: 비거래일/누락일 로그 기록 및 재수집, 반복 실패 시 백업 소스로 보강
- 스플릿/배당: 조정종가(adjusted) 기반으로 수익률/지표 계산
- 상폐/종목변경: 더 이상 수집 불가 시 유니버스에서 제외, 기존 포지션은 강제 청산 시뮬레이션
- 시간대/통화: UTC 표준, 통화 단위는 USD(변경 가능)
...
### 주 1: 기반 + 데이터/지표
- [ ] FastAPI 스캐폴딩(health, bars, indicators)
- [ ] DB 스키마(주식/일봉/지표) 생성, 인덱스 `(stock_id,date)`
- [ ] yfinance 일봉 수집 서비스, 기본 지표(RSI/EMA/MACD)
- [ ] React 초기화(대시보드, 주식 상세 스켈레톤)

### 주 2: 신호 + 경량 백테스트
- [ ] 규칙 기반 신호 생성(히스테리시스)
- [ ] 경량 백테스터(바-클로즈, 고정 수수료/슬리피지)
- [ ] 성과지표 산출(Sharpe/MDD/승률/PF)
- [ ] RF 베이스라인 훈련/저장/추론 배치(장 마감 후)

### 주 3: 가상 거래(MVP)
- [ ] 가상계정/주문/포지션/거래내역 모델 + API(`/vt/*`)
- [ ] Market on Close 체결, 포지션 사이징 고정비율
- [ ] 리스크 가드레일(일손실/편중/연속손실) 적용
- [ ] 프론트: 가상 거래 UI(계정/주문/포지션/내역)

### 주 4: 자동 실행 + 리포트
- [ ] 일일 스케줄 실행(수집→지표→신호→예측→백테스트 요약)
- [ ] 전략 파라미터 관리(환경/DB)
- [ ] 일일 성과 리포트 생성(요약 KPI, 에쿼티커브)
- [ ] 안정화/버그픽스/테스트 보강

Stretch(주 5~8):
- [ ] LSTM/GRU, 신호 가중 앙상블, 분봉 전환, WebSocket, Celery/Redis, 실시간 대시보드

## 🧪 리스크와 품질 게이트
- 데이터 누수 방지: 시계열 분리(TimeSeriesSplit), 최신 구간 홀드아웃 유지
- 체결/비용 일관성: 백테스트와 가상거래 엔진 공통 로직 사용
- 리스크 가드레일: 1일 손실 한도, 연속 손실 중단, 종목/섹터 편중 제한
- 회귀 테스트: 주요 전략/파라미터 변경 시 KPI 하락 경고

## ✅ Exit Criteria(주차별)
- 주1: `/health`,`/stocks/{symbol}/bars`,`/stocks/{symbol}/indicators` 응답 OK, DB 스키마/인덱스 적용, 일봉 수집+RSI/EMA/MACD 계산, 대시보드 기본 렌더
- 주2: 규칙 신호+히스테리시스 생성, 바-클로즈 백테스터 동작, Sharpe/MDD/승률/PF 산출, RF 베이스라인 일일 배치 저장
- 주3: `/vt/*`(계정/주문/포지션/내역) 동작, MOC 체결 비용/슬리피지 반영, 리스크 가드레일 작동, 거래 UI 작동
- 주4: 일일 스케줄(수집→지표→신호→예측→요약) 자동 실행, 파라미터 관리, 일일 리포트 생성, 주요 테스트 통과

## 📋 상세 작업 분할

### **Task 1.1: 프로젝트 루트 폴더 생성**
- **목표**: StockVision 프로젝트의 기본 폴더 구조 생성
- **작업 내용**:
  - [ ] `backend/` 폴더 생성
  - [ ] `frontend/` 폴더 생성
  - [ ] `docs/` 폴더 생성
  ...
  - [ ] `models/` 폴더 생성
  - [ ] `tests/` 폴더 생성
- **완료 기준**: 6개 폴더가 모두 생성되었는지 확인
- **예상 소요 시간**: 5분
- **의존성**: 없음

### **Task 1.2: Git 저장소 초기화**
- **목표**: Git 버전 관리를 위한 저장소 설정
- **작업 내용**:
  - [ ] `git init` 실행
  - [ ] `.gitignore` 파일 생성 (Python, Node.js, 환경변수 등)
  - [ ] 첫 번째 커밋 생성
- **완료 기준**: `git status`로 깨끗한 상태 확인
- **예상 소요 시간**: 10분
- **의존성**: Task 1.1 완료

### **Task 1.3: Python 가상환경 설정**
- **목표**: Python 백엔드 개발을 위한 격리된 환경 구축
- **작업 내용**:
  - [ ] `python -m venv venv` 실행
  - [ ] 가상환경 활성화 스크립트 생성
  - [ ] 가상환경 활성화 테스트
- **완료 기준**: `which python`이 venv 내부 경로를 가리키는지 확인
- **예상 소요 시간**: 15분
- **의존성**: Task 1.1 완료

### **Task 1.4: Node.js 환경 확인**
- **목표**: React 프론트엔드 개발 환경 준비
- **작업 내용**:
  - [ ] `node --version` 확인
  - [ ] `npm --version` 확인
  - [ ] `npx --version` 확인
- **완료 기준**: Node.js 18+ 버전이 설치되어 있는지 확인
- **예상 소요 시간**: 5분
- **의존성**: 없음
...
**문서 버전**: 2.0  
**주요 업데이트**: 문서 통합 완료, 가상 투자 시스템 포함, 상세 작업 분할 추가
