...
| # | 항목 | spec | 의존 | 상태 |
|---|------|------|------|------|
| C1 | 일일 P&L API + OpsPanel 표시 | `spec/ops-panel-v2/spec.md` | 없음 | 미착수 |
| C2 | 운영 패널 확장 (드롭다운, 상세 상태) | `spec/ops-panel-v2/spec.md` | C1 | 미착수 |
| C3 | 실행 로그 타임라인 (trigger→submit→fill/fail) | `spec/execution-log-timeline/spec.md` | System Trader (완료) | 미착수 |
| C4 | exe 패키징 (PyInstaller + 딥링크) | `spec/local-exe-deeplink/spec.md` | 없음 | 구현 완료 (빌드 검증 잔여) |
| C5 | 온보딩 신뢰 강화 (설치 피드백) | `spec/onboarding-v2/spec.md` | C4 | 미착수 |
