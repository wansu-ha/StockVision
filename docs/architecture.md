...
> 최종 갱신: 2026-03-05 | 3프로세스 아키텍처 통합 (API+데이터 서버 병합)
...
| 2 | **클라우드 서버** | 클라우드 | 인증, 규칙 CRUD, 시세 수집/저장, AI 분석, 어드민 |
...
7. 프론트엔드 → 클라우드 서버: 성공/실패만 보고 (금액 미포함)
```
...
| 신호등 | 확인 방법 |
|--------|----------|
| 🟢🔴 클라우드 서버 | 프론트가 직접 health check |
| 🟢🔴 로컬 서버 | 클라우드 서버가 하트비트로 판단, 또는 프론트가 localhost 직접 확인 |
...
| 상황 | 처리 |
|------|------|
| 클라우드 서버 장애 | 로컬 서버 캐시된 규칙으로 계속 실행 |
