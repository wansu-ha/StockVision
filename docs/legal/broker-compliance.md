...
**목적**: 키움증권 OpenAPI+ 및 한국투자증권 Open Trading API 이용약관 준수 여부 자체 검증
...
**Part II: 한국투자증권 Open Trading API** (추가)
7. KIS 약관 준수 개요
8. KIS 고유 준수 사항
...
[사용자 PC]
  ...
  │   └─ 증권사 유저 API Key/Secret (키움 또는 KIS)
  │
...
## Part II: 한국투자증권 Open Trading API 준수

### 7. KIS 약관 준수 개요

한국투자증권 Open Trading API를 추가 지원함에 따라, 해당 약관 준수 사항을 아래에 정리합니다.
키움증권과의 핵심 차이점만 기술하며, **공통 아키텍처 원칙(Key 분리, 로컬 실행, 시세 미중계)은 동일하게 적용**됩니다.

### 7.1 KIS API Key 관리

| Key 종류 | 소유자 | 저장 위치 | 접근 가능자 |
|---------|-------|---------|-----------|
| **서비스 키** | StockVision | 클라우드 서버 (환경변수) | 시세 수집 전용 |
| **유저 키** | 각 사용자 | 사용자 PC (Credential Manager) | 로컬 서버만 |

- KIS의 `appkey` / `appsecret` → 키움의 API Key/Secret에 대응
- 토큰 발급 (`/oauth2/tokenP`) 결과인 `access_token`은 로컬 메모리에만 유지
- 토큰 만료(24시간) 시 자동 재발급, 클라우드 미전송

### 7.2 KIS 고유 약관 조항

| KIS 약관 | 요건 | 우리 대응 |
|---------|------|---------|
| 제3자 제공 금지 | API 키를 제3자에게 제공 금지 | 사용자 PC에만 저장 ✅ |
| 시세 재배포 금지 | 수신 시세 외부 제공 금지 | 분석 결과만 제공 ✅ |
| 보안 관리 의무 | 접근 토큰 안전 관리 | Credential Manager + DPAPI ✅ |
| 호출 빈도 제한 | 초당 20건 등 제한 | Rate limiter 적용 ✅ |

### 7.3 키움 vs KIS 비교

| 항목 | 키움 OpenAPI+ | KIS Open Trading API |
|------|-------------|---------------------|
| 인증 방식 | API Key + Secret → Token | appkey + appsecret → Token |
| 토큰 유효기간 | 세션 기반 | 24시간 |
| 시세 수신 | WebSocket | WebSocket |
| 주문 실행 | REST API | REST API |
| IP 제한 | 화이트리스트 지원 | 화이트리스트 지원 |
| 코드 위치 | `local_server/broker/kiwoom/` | `local_server/broker/kis/` |

### 8. KIS 체크리스트

- [ ] KIS appkey/appsecret을 클라우드에 저장하지 않음
- [ ] KIS access_token을 로컬 메모리에만 유지 (디스크 미저장)
- [ ] KIS 시세를 클라우드로 전송하지 않음
- [ ] KIS 호출 빈도 제한 (Rate limiter) 구현
- [ ] KIS 약관 변경 시 분기별 검토
...
- **자본시장과 금융투자업에 관한 법률(자본시장법)**: 증권회사의 책임 제한
- **개인정보보호법**: 개인정보 보호 의무
- **정보통신망법**: 개인정보 보호
