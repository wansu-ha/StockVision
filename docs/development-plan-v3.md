...
> 작성일: 2026-03-15 | 갱신: 2026-03-24 (T3 D2 완료, Phase A/T1 다이어그램 정정) | 상태: 확정
...
>
> **2026-03-19 갱신**: 전수 코드-문서 감사 2차 수행. relay-infra Step 1~8 전부 구현 완료 확인 → spec 상태 `구현 완료`로 갱신. F2 staleTime 현황 정정: 20/46 쿼리 설정, 26건 잔여. A9 ListView 키 미등록 온보딩 미구현 확인. Sprint 4건 착수 (F2 전수, Q5, F3, R4).
...
#### A6. frontend-quality (F1~F3) — ✅ 구현 완료 (2026-03-19)

| Step | 항목 | 파일 | 상태 |
|------|------|------|------|
| F1 | ErrorBoundary | `ErrorBoundary.tsx`, `App.tsx` | ✅ (라우트 리셋 구현 완료, 2026-03-18) |
| F2 | staleTime 설정 | 다수 (12+ 파일) | ✅ 46/46 쿼리 전수 설정 완료 (2026-03-19) |
| F3 | 프로필 수정 (닉네임) | `auth.py`, `Settings.tsx` | ✅ GET/PATCH /profile + Settings UI (2026-03-19) |
...
| Q5 | 장 상태 공휴일 (useMarketContext 활용) | `MainDashboard.tsx` | ✅ 구현 완료 (2026-03-19) |
...
| F2+ | staleTime 전수 정리 | 다수 파일 | ✅ 전체 46/46 쿼리 설정 완료 (2026-03-19, Sprint Stage 1) |
| D1 | .env 플랜 상수 추가 | `core/config.py` | ✅ 구현 완료 (2026-03-18) |
| D4 | 메신저 드롭다운 UI 선점 | `AlertSettings.tsx` | ✅ 구현 완료 (2026-03-18) |

**A8 전건 완료 (Q5 공휴일 + A6 F2/F3 구현 완료, 2026-03-19)**
...
- [x] 품질 이슈: F2 staleTime 전수 완료, F3 닉네임, Q5 공휴일 (2026-03-19)
- [x] Q1~Q4, Q6 구현 완료 확인 (2026-03-17), Q7 다크 테마 구현 완료 확인 (2026-03-17)
- [x] broker-auto-connect 프론트 연동 완료 (2026-03-16)
- [x] 프로필 수정 (F3) 구현 (2026-03-19)
- [x] `npm run build` 경고 없음 (2026-03-24, 메인 청크 1263KB → 373KB)
- [x] `npm run lint` 통과 (2026-03-24)
...
#### T1-4. chart-timeframe Stage 3 (프론트엔드) — ✅ 구현 완료 (코드 감사 확인 2026-03-24)
...
- [x] 일/주/월봉 타임프레임 전환 + 분봉 UI (2026-03-24, 코드 감사 완료)
...
| 문서 | 이슈 | 상태 |
|------|------|------|
| Phase 1/2 문서 5건 | SUPERSEDED 헤더 추가 | ✅ 완료 (2026-03-24) |
| architecture.md | Phase A~T2 추가 기능 반영 | ✅ 완료 (2026-03-24) |
...
Phase A (Week 1-3) — ✅ 완료 ═════════════════════════════════════
  ...
  ├─ [A6] frontend-quality F1~F3              ✅ 전부 완료
  ├─ [A7] legal L1→L2→L3                   ✅
  ├─ [A8] 품질 이슈 Q1~Q7                   ✅ 전부 완료
  └─ [A9] broker-auto-connect               ✅

T1 (Week 4-6) ════════════════════════════════════════════════════

  Week 3-4 (병렬):
  ┌─ [T1-1] engine-live-execution             ✅ 완료 (2026-03-24)
  ├─ [T1-2] dsl-client-parser D1→D2→D3→D4    ✅ 완료
  └─ [T1-3] chart-timeframe Stage 1+2         ✅ 완료

  Week 5 (병렬 + 순차):
  ┌─ [T1-4] chart-timeframe Stage 3           ✅ 완료 (코드 감사 확인 2026-03-24)
