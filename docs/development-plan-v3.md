...
> 작성일: 2026-03-15 | 갱신: 2026-03-17 (T2 auth-extension 추가, relay/remote-ops 정합성 보완) | 상태: 확정
...
| Step | 항목 |
|------|------|
| 1 | 클라우드 WS `/ws/relay` (로컬 서버 전용, JWT 인증, 연결 레지스트리) |
| 2 | 로컬 WS 클라이언트 (ws_relay_client.py 재작성, 재연결 backoff) |
| 3 | Heartbeat WS 전환 (HTTP 폴백 유지) |
| 4 | 메시지 프로토콜 + 라우팅 (state/alert/command_ack) |
| 5 | 클라우드 WS `/ws/remote` (디바이스 전용, SessionManager) |
| 6 | E2E 암호화 (AES-256-GCM, Python + TypeScript) |
| 7 | 오프라인 명령 큐 (PendingCommand DB) |
| 8 | 감사 로그 + Rate Limiting |
