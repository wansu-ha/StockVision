...
> 조사일: 2026-03-08
> 업데이트: 2026-03-12 (젠포트 공식 랜딩의 AI 전략 생성/원스톱 흐름 반영)
...
- StockVision은 "DIY 도구 + Cloud AI 데이터 가공" (패턴 B+C) 영역
- Cloud AI가 데이터를 가공(감성 점수, 뉴스 요약)하고, 사용자가 로컬에서 판단(DSL 규칙 또는 Custom LLM)

### 6.3 커스텀 모델 지원은 상위 사용자를 위한 차별점
- OpenBB의 BYOLLM, QuantConnect의 ML 프레임워크가 모범 사례
- StockVision은 DSL 규칙 엔진(v1) + Custom LLM(v2) 기반이므로, 사용자 LLM 연결이 곧 커스텀 모델 지원에 해당
...
| **LLM 연동** | B2B만 | 내부 적용 | 불가 | 미정(카이드라 예정) | 내장 AI 전략 생성, 사용자 LLM 직접 연동 미확인 | 미확인 | 웹훅 우회 가능 |
...
| **AI 역할** | 종목선정~매매 전권 | 전략 설계 보조 + 백테스트/자동매매 인프라 (서비스별 차이) | 차트·지표 도구 |
...
StockVision = "Cloud AI가 데이터 가공, 니가 로컬에서 판단" (DIY + Cloud AI 보조)
```

- 젠포트도 AI 전략 생성을 전면에 내세우기 시작해 `LLM 유무`만으로는 차별화가 어렵다.
- StockVision의 포지셔닝은 `젠포트형 DIY + 로컬 실행 + 사용자 LLM 직접 연결 + 오픈 검증 가능성` 쪽이 더 정확하다.
- 자연어로 전략 → 백테스트 가능한 코드 변환 → 가상매매 검증 흐름은 이제 시장에서 부분적으로 검증된 요구다.
- 일임업 라이선스 없이도 서비스 가능 (사용자가 최종 매매 버튼)
- 젠포트 대비 핵심 차별화 후보는 조건식 설계 보조 자체보다 `로컬 신뢰 구조`와 `사용자 모델 선택권`이다.
...
- [젠포트 공식 랜딩](https://genport.newsystock.com/)
