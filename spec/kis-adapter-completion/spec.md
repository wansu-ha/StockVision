...
**문제**: 현재 KIS REST 요청마다 `appsecret` 헤더를 포함하여 전송한다.
KIS API 문서상 `appsecret`은 `/oauth2/token` (토큰 발급)에만 필요하며,
일반 API 호출에는 `access_token`만 있으면 된다. 불필요하게 secret이 네트워크에 노출된다.

**현재 코드** (`local_server/broker/kis/auth.py` 공통 헤더):
```python
headers = {
    "authorization": f"Bearer {token}",
    "appkey": self._app_key,
    "appsecret": self._app_secret,  # ← 불필요한 전송
}
```

**요구사항**:
- `/oauth2/token`, `/oauth2/Approval` 요청에만 `appsecret` 포함
- 일반 REST 요청 (주문, 시세 등)에서는 `appsecret` 제거
- `appkey`는 KIS API가 요구하므로 유지
...
| `local_server/broker/kis/auth.py` | K3: 공통 헤더에서 `appsecret` 제거 |
...
- [ ] 일반 REST 요청 헤더에 `appsecret`이 포함되지 않는다
- [ ] `/oauth2/token` 요청에만 `appsecret`이 포함된다
