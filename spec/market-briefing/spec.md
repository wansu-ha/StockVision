...
- [ ] 매일 06:00 KST 자동 생성 (스케줄러 job 추가 확인)
- [ ] `GET /api/v1/ai/briefing` 응답 200, JSON 구조 정확
- [ ] Redis 캐시 적중 시 동일 날짜 두 번째 호출이 Claude 미호출 (source: "cache")
- [ ] 과거 날짜 조회 시 DB에서 반환 (Claude 미호출)
- [ ] Claude API 키 없을 때 스텁 반환 — 앱 에러 없음
- [ ] 프론트엔드 브리핑 카드 렌더링 (텍스트 + 지수 3종)
- [ ] 로딩/에러/정상 상태 모두 처리
