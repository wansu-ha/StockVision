...
- [ ] 장 시간(09:00~15:30)에만 1분 주기로 evaluate_all 호출
- [ ] 장외 시간에는 실행하지 않음
...
- [ ] RSI <= 30 AND price <= 50000 → True 반환 (데이터 충족 시)
- [ ] OR 조건 정상 동작
- [ ] 조건 파싱 오류 → 해당 규칙만 비활성화 + 로그

### 6.3 중복 방지

- [ ] 같은 규칙이 하루에 2회 이상 실행되지 않음
- [ ] 자정 기준 리셋 → 다음날 재실행 가능

### 6.4 가격 검증

- [ ] WS 가격과 REST 가격 괴리 < 1% → 주문 실행
- [ ] 괴리 > 1% → 주문 거부 + 로그 기록

### 6.5 주문 실행

- [ ] 모의투자 시장가 매수 → 체결 성공 → logs.db 기록
- [ ] 체결 알림 → localhost WS로 프론트엔드에 전송

### 6.6 한도

- [ ] 일일 예산(budget_ratio) 초과 시 주문 스킵
- [ ] 최대 포지션 수 초과 시 주문 스킵

### 6.7 안전장치

**Kill Switch 2단계:**
- [ ] STOP_NEW — 신규 주문 차단, 기존 미체결 유지
- [ ] CANCEL_OPEN — 신규 차단 + BrokerAdapter로 미체결 전량 취소
- [ ] 트레이/프론트에서 단계 선택 가능 (기본: STOP_NEW)
- [ ] Kill Switch 해제 → 수동만 허용 (자동 재개 금지)
- [ ] Kill Switch 이벤트 logs.db 기록 (활성화/해제/단계)

**최대 손실 제한:**
- [ ] 기준: 당일 실현손익만 (평가손익 미포함, v1)
- [ ] 데이터: 로컬 체결 로그(logs.db) 기반 당일 실현손익 합산
- [ ] 발동 시: CANCEL_OPEN 자동 실행 + 트레이/프론트 알림
- [ ] 해제: 수동만 ("락" — 사용자가 직접 풀기 전까지 Trading Enabled=OFF 유지)
- [ ] 해제 절차: 트레이 메뉴 "손실 락 해제" 또는 프론트 대시보드(`POST /api/strategy/unlock`) — 두 경로 외 자동 해제 없음
- [ ] 해제 시 logs.db에 해제 이벤트 기록 (시각, 해제 경로, 당시 실현손익)

**주문 속도 제한:**
- [ ] 분당 주문 수 > 한도(기본 10건/분) → 초과분 차단 + 경고 로그

### 6.8 분봉 구성

- [ ] WS 시세로 1분 OHLCV 구성 (키움 서버 시간 기준 분 경계)
- [ ] 장 시작 직후 (09:00~09:02) SYNCING 상태 → 평가 보류
