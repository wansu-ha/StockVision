...
### WS 연결
- [x] 로컬 서버 시작 시 클라우드 WS에 자동 연결된다
- [x] 연결 끊김 시 exponential backoff로 자동 재연결된다
- [x] 기존 하트비트 데이터가 WS heartbeat 메시지로 전달된다
- [x] 클라우드가 하트비트 응답에 버전 정보를 포함한다 (기존 기능 유지)
...
### 메시지 프로토콜
- [x] 모든 메시지가 envelope 형식(v, type, id, ts, payload)을 따른다
- [ ] 디바이스 오프라인 후 재연결 시 last_received_ts 기반 갭 채우기가 동작한다

### 오프라인 명령 큐
- [ ] 로컬 오프라인 시 명령이 클라우드 DB에 저장된다
- [x] 로컬 재연결 시 pending 명령이 WS로 flush된다
- [x] 실행 후 ACK로 큐에서 제거된다

### 세션 관리
- [x] 동시 디바이스 수가 제한된다 (기본 5대)
- [x] ping/pong 무응답 시 세션이 종료된다
- [x] JWT 만료 시 재인증이 요구된다
- [x] 특정 디바이스 세션을 강제 종료할 수 있다

### 감사 로그
- [x] 모든 명령이 감사 로그에 기록된다 (사용자, 디바이스, 시각, IP)

### Rate Limiting
- [x] 명령 초과 시 429 응답이 반환된다
