...
- [x] BUY 체결 시 파란 arrowUp 마커가 봉 아래에 표시된다
- [x] SELL 체결 시 빨간 arrowDown 마커가 봉 위에 표시된다
- [x] ~~거부/실패 시 노란 circle 마커가 표시된다~~ → v2에서 제거 (로그 화면에서만 확인)
- [x] 차트 타입 전환 후에도 마커가 유지된다
- [x] 기간 변경 시 해당 기간의 마커만 표시된다
- [x] 체결 데이터가 없으면 마커 없이 정상 렌더링된다
...
| 변경 | 이전 | 이후 |
|------|------|------|
| 실패 마커 | 차트에 주황 원으로 표시 | 제거 — 로그 화면에서만 확인 |
| 마커 크기 | 기본 (size 미지정) | `size: 0.5` (축소) |
| 같은 날 중복 | 개별 표시 (겹침) | 합산 표시 ("매수×3") |
