...
> 작성일: 2026-03-15 | 상태: 구현 완료 | 갱신: 2026-03-18
...
- [x] `parseDsl("매수: rsi_14 > 30 AND macd < 0\n매도: price < 50000")`이 올바른 구조를 반환한다
- [x] `dslToConditions()` → `conditionsToDsl()` round-trip이 동일 결과를 생성한다
- [x] 서버에 저장된 기존 규칙을 프론트에서 폼 모드로 편집할 수 있다
- [x] 문법 에러가 있는 DSL 입력 시 에러 위치와 메시지가 표시된다
- [x] 폼 모드 ↔ 스크립트 모드 전환 시 데이터가 유지된다
- [x] 변환 불가능한 복잡 DSL은 스크립트 모드에서만 편집 가능하다
