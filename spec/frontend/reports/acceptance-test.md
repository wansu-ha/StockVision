...
| AND/OR 연산자 선택 가능 | **PASS** | 복수 조건 + AND/OR 토글 구현 완료 (`DetailView.tsx` AddRuleModal) |
...
- 복수 조건: "+ 조건" 버튼으로 행 추가, 개별 삭제 가능
- AND/OR 토글: 2개 이상 조건 시 AND/OR 스위치 표시
- 지표(9종) + 비교연산자(≤/≥/=) + 값
- 실행: 매수/매도 + 수량 + 시장가/지정가
...
**12/13 PASS, 1/13 PARTIAL**

- 전체 13개 수용 기준 중 12개 통과 (UI 확인 또는 코드 확인)
- 1개 부분 통과: "모의투자/실거래 모드 전환" — 상태 표시는 구현, 토글 UI 없음 (설계상 키 타입으로 결정)
- AND/OR 복수 조건: 구현 완료 (DetailView.tsx AddRuleModal)
