...
**수용 기준**:
- [ ] 에러 발생 후 사이드바 네비게이션으로 다른 페이지 이동 시 에러 화면 사라짐
- [ ] 같은 페이지 내 에러는 "다시 시도" 버튼으로 리셋 가능 (기존 동작 유지)

---

### F2+ — staleTime 미설정 쿼리

**현재**: React Query의 `useQuery` 호출 중 7건에 `staleTime`이 없다.
기본값 `0`이 적용되어 컴포넌트 마운트마다 불필요한 refetch 발생.
...
**수용 기준**:
- [ ] 위 7건 모두 staleTime 설정됨
- [ ] 탭 전환/컴포넌트 재마운트 시 staleTime 내 데이터는 캐시 사용
...
**수용 기준**:
- [ ] `settings.PLAN_MAX_STRATEGIES`, `settings.PLAN_HISTORY_DAYS` 참조 가능
- [ ] 환경변수 미설정 시 무료 플랜 기본값 사용
...
**수용 기준**:
- [ ] "알림 채널" 드롭다운이 AlertSettings 상단에 표시
- [ ] 앱 내 알림이 기본 선택
- [ ] 이메일/텔레그램/디스코드는 "준비 중" 표시 + 선택 불가
- [ ] 기존 알림 규칙 토글 기능에 영향 없음
