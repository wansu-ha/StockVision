...
- [ ] 가상 계좌 생성 후 매수/매도 주문이 정상 처리되고 잔고가 정확히 반영된다
- [ ] 스코어링 결과가 0~100 범위로 산출되고, 기존 기술적 지표 데이터를 활용한다
- [ ] 백테스팅이 과거 데이터로 전략을 시뮬레이션하고 수익률/승률/샤프비율을 출력한다
- [ ] 자동매매 규칙 등록 후 스케줄에 따라 스코어링→매수→매도가 자동 실행된다
- [ ] 프론트엔드에서 계좌 현황, 스코어링 결과, 백테스팅 결과를 조회할 수 있다
- [ ] 키움증권 REST API로 실시간 시세를 조회할 수 있다
- [ ] 모든 API가 기존 `{ success, data, count }` 응답 형식을 따른다
